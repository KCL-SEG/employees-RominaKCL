...
class Employee:
    ...
    def __str__(self):
        if self.salary_Contract:
            ...
            self.name += "contract of " + str(self.work_hours)+" hours at "+str(self.hourly_wage)+"/hour"
        if self.commission:
            self.name += " and receives a "
            if self.fixed_comssion:
                self.name += "bonus commission of "+str(self.fixed_bonus)
            else:
                self.name += "commission for " + str(self.contract_numbers)+" contract(s) at " + str(self.contract_commission)+"/contract"
